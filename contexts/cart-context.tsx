...
import type { MenuItem, SelectedSauce, SelectedCheese } from "@/lib/types";
...
interface CartContextType {
  ...
  currency: string;
  addItem: (item: MenuItem, quantity?: number, selectedSauces?: SelectedSauce[], selectedCheeses?: SelectedCheese[]) => void;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const generateCartItemKey = useCallback((itemId: string, sauces?: SelectedSauce[], cheeses?: SelectedCheese[]) => {
    const sauceKey = sauces?.map(s => s.sauceType).sort().join('-') || '';
    const cheeseKey = cheeses?.map(c => c.cheeseType).sort().join('-') || '';
    return `${itemId}_${sauceKey}_${cheeseKey}`;
  }, []);
  ...
  const addItem = useCallback((
    ...
    const cartItemKey = generateCartItemKey(item.id, selectedSauces, selectedCheeses);
  ...
  const submitOrder = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const total = items.reduce((sum, ci) => {
        ...
        return sum + (itemPrice + saucesExtra + cheesesExtra) * ci.quantity;
      }, 0);
      ...
      const payload: CreateOrderPayload = {
        ...
        items: items.map((cartItem) => {
          ...
          return {
            ...
            price: itemPrice + saucesExtra + cheesesExtra,
  ...
  const totalPrice = useMemo(
    () => items.reduce((sum, ci) => {
      ...
      const saucesExtra = ci.selectedSauces?.reduce((s, sauce) => s + (sauce.extraCost || 0), 0) || 0;
      const cheesesExtra = ci.selectedCheeses?.reduce((s, cheese) => s + (cheese.extraCost || 0), 0) || 0;
      const itemTotal = (itemPrice + saucesExtra + cheesesExtra) * ci.quantity;
